...
export default function CleaningRecentActivity({ incomeEntries, expenseEntries, mileageEntries, onLongPressIncome, onLongPressExpense, onLongPressMileage }) {
  ...
  const mileage = [...(mileageEntries || [])].filter(r => !r.deleted).sort((a, b) => b.date - a.date);
  ...
  return (
    <View>
      ...
      {mileage.length === 0
        ? <Text style={styles.empty}>No mileage logged</Text>
        : mileage.map(item => (
          <TouchableOpacity key={item.id} style={styles.row} onLongPress={() => onLongPressMileage(item)} delayLongPress={400}>
            <Text style={styles.date}>{fmt(item.date)}</Text>
            <Text style={[styles.meta, { flex: 1 }]} numberOfLines={1}>{item.purpose || '—'}</Text>
            <Text style={styles.meta}>{(item.miles || 0).toFixed(1)} mi</Text>
            <Text style={[styles.amount, { color: theme.statusPositive }]}>{formatCentsShort(item.deductionCents)}</Text>
          </TouchableOpacity>
        ))}
    </View>
  );
